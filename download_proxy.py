...
if __name__ == '__main__':
    ...
    try:
        batch = download_proxy(share_url, output_path)
    except Exception as error:
        batch = [{"status": "FAIL", "error_type": type(error).__name__}]
    print(json.dumps(batch, ensure_ascii=False, indent=2))
    raise SystemExit(0 if all(item["status"] == "PASS" for item in batch) else 1)
