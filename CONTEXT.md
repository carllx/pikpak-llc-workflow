...
* **Job Workspace**：一次 Share invocation 创建一个 `workspace/jobs/<job-id>/`。运行中的 workflow 以显式 Job identity 为 SSOT；`workspace/LATEST.txt` 仅作为缺省时的默认恢复指针，不得作为并发运行任务的动态身份来源。一个 Job 可包含多个 video proxies 与对应的多个 LLC；Origin 输出按 `segments/<source-stem>/` 隔离。
